...
import { Plus, MapPin, Calendar, Weight, DollarSign, User, Package } from 'lucide-react';
...
const TravellerTrips: React.FC = () => {
  const { trips, bookings, fetchTrips, updateTripStatus, isLoading } = useTripStore();
  ...
  useEffect(() => {
    if (trips.length > 0 && user) {
      const filteredTrips = trips.filter(trip => trip.travellerId === user.id);
      ...
      {getFilteredTrips().length === 0 ? (
        ...
        <div className="space-y-8">
          {getFilteredTrips().map((trip) => {
            ...
            const totalBooked = tripBookings.reduce((sum, booking) => sum + booking.luggageSize, 0);
            const totalEarnings = tripBookings.reduce((sum, booking) => sum + booking.totalPrice, 0);
              ...
              <Card key={trip.id}>
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div className="space-y-4">
                      ...
                      <div className="flex items-start">
                        ...
                        <div className="ml-2">
                          ...
                          <p className="text-base font-medium text-gray-900">
                            {trip.availableSpace} kg total ({totalBooked} kg booked, {trip.availableSpace - totalBooked} kg available)
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-start">
                        ...
                        <div className="ml-2">
                          <p className="text-sm text-gray-500">Price & Earnings</p>
                          <p className="text-base font-medium text-gray-900">
                            {trip.pricePerKg} {trip.currency} per kg (Total earnings: {totalEarnings} {trip.currency})
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div>
                      <h3 className="text-lg font-medium text-gray-900 mb-3">Bookings ({tripBookings.length})</h3>
                          ...
                          {tripBookings.map((booking) => (
                            <div key={booking.id} className="bg-gray-50 p-3 rounded-md">
                              <div className="flex items-start">
                                ...
                                <div className="ml-2">
                                  <p className="text-sm font-medium text-gray-900">{booking.senderName}</p>
                                  <p className="text-xs text-gray-500">
                                    {booking.luggageSize} kg - {booking.totalPrice} {trip.currency}
                                  </p>
                                  <p className="text-xs text-gray-500 mt-1">{booking.luggageDescription}</p>
