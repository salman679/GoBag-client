...
import { Users, Briefcase, CreditCard, Star, BarChart2, Settings, User, Package, DollarSign } from 'lucide-react';
...
const AdminDashboard: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'users' | 'trips' | 'bookings' | 'payments' | 'reviews'>('overview');
  ...
  const totalRevenue = bookings.reduce((sum, booking) => sum + booking.totalPrice * 0.1, 0); // 10% platform fee
          ...
          {activeTab === 'overview' && (
            <div>
                ...
                <Card className="bg-gradient-to-br from-blue-500 to-blue-600 text-white">
                  <CardContent className="p-6">
                    ...
                    <div className="mt-4 text-blue-100 text-sm">
                      <span className="text-white font-medium">+12%</span> from last month
                    </div>
                  </CardContent>
                </Card>
                ...
                <Card className="bg-gradient-to-br from-green-500 to-green-600 text-white">
                  <CardContent className="p-6">
                    ...
                    <div className="mt-4 text-green-100 text-sm">
                      <span className="text-white font-medium">+8%</span> from last month
                    </div>
                  </CardContent>
                </Card>
                ...
                <Card className="bg-gradient-to-br from-purple-500 to-purple-600 text-white">
                  <CardContent className="p-6">
                    ...
                    <div className="mt-4 text-purple-100 text-sm">
                      <span className="text-white font-medium">+15%</span> from last month
                    </div>
                  </CardContent>
                </Card>
                ...
                <Card className="bg-gradient-to-br from-yellow-500 to-yellow-600 text-white">
                  <CardContent className="p-6">
                    ...
                    <div className="mt-4 text-yellow-100 text-sm">
                      <span className="text-white font-medium">+20%</span> from last month
                    </div>
                  </CardContent>
                </Card>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <Card>
                  <CardHeader>
                    <h3 className="text-lg font-medium text-gray-900">Trip Statistics</h3>
                  </CardHeader>
                  <CardContent>
                          ...
                          <tr key={i}>
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="flex items-center">
                                <div className="flex-shrink-0 h-10 w-10">
                                  <img className="h-10 w-10 rounded-full" src={`https://images.unsplash.com/photo-${1570295999919 + i}-b8c36b15f770`} alt="" />
                                </div>
                                <div className="ml-4">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                              <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                                ...
                                {i % 3 === 0 ? 'Admin' : i % 3 === 1 ? 'Sender' : 'Traveller'}
                              </span>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                              <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                                i % 5 === 0 ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'
                              }`}>
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {trips.slice(0, 5).map((trip) => (
                          <tr key={trip.id}>
                              ...
                              <div className="flex items-center">
                                <div className="flex-shrink-0 h-8 w-8">
                                  {trip.travellerProfilePic ? (
                                    <img className="h-8 w-8 rounded-full" src={trip.travellerProfilePic} alt="" />
                                  ) : (
                                ...
                                <div className="ml-3">
                                  <div className="text-sm font-medium text-gray-900">{trip.travellerName}</div>
                                </div>
                              </div>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                              {bookings.filter(b => b.tripId === trip.id).length} bookings
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
            ...
            <div className="text-center py-12 bg-white rounded-lg shadow-md">
              <h2 className="text-xl font-bold text-gray-900 mb-4">
                {activeTab === 'bookings' ? 'Booking Management' : 
                 activeTab === 'payments' ? 'Payment Management' : 'Review Management'}
              </h2>
              <p className="text-gray-600 mb-6">
